...
const motherboardDetails = ({ motherboardData }) => {
  return (
    <div>
      <div>
        <section class="text-gray-600 body-font overflow-hidden">
          <div class="container px-5 py-24 mx-auto">
            <div class="lg:w-4/5 mx-auto flex flex-wrap">
              <img
                src={motherboardData?.image}
                alt=""
                class="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded"
              />
              <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                <h2 class="text-sm title-font text-gray-700 tracking-widest">
                  {motherboardData?.category}
                </h2>
                <h1 class="text-gray-900 text-3xl title-font font-medium mb-1">
                  {motherboardData?.name}
                </h1>
                <div class="flex mb-4">
                  <span class="flex items-center">
                    IR: {motherboardData?.indivisualrating}
                    <span class="text-gray-600 ml-3">
                      AR: {motherboardData?.averagerating}
                    </span>
                  </span>
                  <span class="flex ml-3 pl-3 py-2 border-l-2 border-gray-200 space-x-2s">
                    {motherboardData?.status}
                  </span>
                </div>
                <p class="leading-relaxed">{motherboardData?.description}</p>
                <div class="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                  <div class="flex">
                    <span class="mr-3">keyfeatures:</span>
                    <p class="ml-1 text-black w-6 h-6">
                      {motherboardData?.keyfeatures}
                    </p>
                  </div>
                </div>
                <div class="flex">
        ...
        <section class="text-gray-600 body-font">
          <div class="container px-5 py-8 mx-auto">
            {motherboardData?.reviews.map((review, index) => (
              <div
                key={index}
...
export const getStaticPaths = async () => {
  ...
  const paths = motherboard.map((motherboard) => ({
...
export const getStaticProps = async (context) => {
  ...
  const res = await fetch(
    `https://pc-builder-backend-server.onrender.com/categories/motherboard/${params.motherboardid}`,
  );
