...
const othersDetails = ({ othersData }) => {
  return (
    <div>
      <div>
              ...
              <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                ...
                <div class="flex mb-4">
                  <span class="flex items-center">
                    IR: {othersData?.indivisualrating}
                    <span class="text-gray-600 ml-3">
                      AR: {othersData?.averagerating}
                    </span>
                  </span>
        ...
        <section class="text-gray-600 body-font">
          <div class="container px-5 py-8 mx-auto">
            {othersData?.reviews.map((review, index) => (
              <div
                key={index}
...
export const getStaticPaths = async () => {
  ...
  const paths = others.map((other) => ({
    params: { othersid: other._id },
  }));
...
export const getStaticProps = async (context) => {
  ...
  const res = await fetch(
    `https://pc-builder-backend-server.onrender.com/categories/others/${params.othersid}`,
  );
